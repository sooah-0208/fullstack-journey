...
```
볼륨 생성은 생략해도 되지만 컨테이너를 지우면 db가 다 날아감  
-> 그래서 웬만하면 volume 생성해야함
...
1. python에서 for문 처리하여 하나씩 넣는 방식
```sql
sql1 = f"""
...
단점
- 행이 많아질 수록 느려짐
- `'`같은 문자를 인식하면 sql문에 오류가나서 해당 data들은 누락될 수 있음
- 보안에 취약함 (중간에 Drop같은 sql문을 악의적으로 집어넣으면 날아갈 수 있음)

2. %s(프레스홀더)로 DB에서 처리하는 방식
```sql
sql2 = f"""
          INSERT INTO edu.`melon` 
...
장점
- 보안에 강함
- 대량 데이터 삽입 시 빠른 작업 가능
- `'`같은 특수문자도 자동 입력 가능함
- `ON DUPLICATE KEY UPDATE`가 값이 없으면 insert, 있으면 update처리해줌
단점
- `%s`의 개수 등을 정확히 맞춰주지 않으면 오류 발생함
