...
운영체제(OS) ubuntu설치 -> 버추얼박스 설치-> 도커 실행 -> 이미지 pull -> 해당 이미지를 쓰는 container 생성 -> 이때 옵션 추가(volume으로 호스트:컨테이너 같은 파일 생성, port로 연결) 여기 옵션에서 같은 network로 만들면 여러 컨테이너가 연결이 됨

[unbunto 설치 사이트](https://ubuntu.com/download/server)
[버추얼박스 설치 사이트](https://www.virtualbox.org/wiki/Downloads)
